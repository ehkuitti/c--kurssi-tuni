...
private:
    ...
    void subdirectoriesRecursively(Directory* dir, int& numberToBeIncreased) const;

    void recursiveFind(Directory *cur_dir, std::set<std::string> &all_paths, std::string current_path, int n, const int& max) const;
    ...
    Directory* working_directory_ = home_;
    std::vector<Directory*> all_;
//    std::vector<Directory*> all_;
};
