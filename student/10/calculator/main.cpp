...
using CommandFunction = double(*)(double, double);
...
int main() {
    ...
    while ( true ) {
        ...
        string uppercase_command = pieces.at(0);
        for (char& c : uppercase_command)
        {
            c = toupper(c);
        }
        ...
        for (
             ...
             ; ++command)
        {
            if (command->str == uppercase_command)
            {
                break;
            }
        }
...
//        else if (pieces.size() - 2 == command->parameter_number)
//        {
...
//        }
        ...
        if (!string_to_double(left_str, left) || !string_to_double(right_str, right))
        {
